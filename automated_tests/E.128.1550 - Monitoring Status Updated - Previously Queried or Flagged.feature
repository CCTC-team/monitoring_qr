Feature: E.128.1500 - The system shall automatically update the monitoring status to 'Requires verification due to data change' if the current status is 'Verification complete' based on the selected option using Monitoring QR external module.
  ...
  Scenario: Enable external module in project
    Given I create a new project named "E.128.1550" by clicking on "New Project" in the menu bar, selecting "Practice / Just for fun" from the dropdown, choosing file "fixtures/cdisc_files/E1281500.xml", and clicking the "Create Project" button
    ...
    And I select "When the updated field has been previously queried or the field is flagged" on the dropdown field labeled "A form's monitoring status is automatically set to 'Requires verification due to data change'"
