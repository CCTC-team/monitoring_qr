Feature: E.128.1500 - The system shall automatically update the monitoring status to 'Requires verification due to data change' if the current status is 'Verification complete' based on the selected option using Monitoring QR external module.
  ...
  Scenario: Enable external module in project
    Given I create a new project named "E.128.1540" by clicking on "New Project" in the menu bar, selecting "Practice / Just for fun" from the dropdown, choosing file "fixtures/cdisc_files/E1281500.xml", and clicking the "Create Project" button
    ...
    And I select "When the updated field has been previously queried" on the dropdown field labeled "A form's monitoring status is automatically set to 'Requires verification due to data change'"
    ...
    # Then I should see "You have not provided any queries."
    ...
    # Alert not triggered in Cypress
    # Then I should see "You have not provided a response for any of the queries. Please provide a response."
    # And I click on the button labeled "OK"
    ...
    # Alert not triggered in Cypress
    # Then I should see "You cannot verify a query that has no response."
    # And I click on the button labeled "OK"
    When I click on the button labeled "Send back for further attention"
    # Alert not triggered in Cypress
    # Then I should see "You cannot accept a response of `No response`. Please select reraise"
    # And I click on the button labeled "OK"
  ...
  Scenario: E.128.100 - Disable external module
    ...
    And I should NOT see "E.128.1510"
    ...
    Then I should see a table header and row containing the following values in a table:
      | Time             | User       | Event                                                                       |
      | mm/dd/yyyy hh:mm | test_admin | Disable external module "monitoring_qr_v1.1.0" for system                   |
      | mm/dd/yyyy hh:mm | test_admin | Disable external module "monitoring_qr_v1.1.0" for project                  |
      | mm/dd/yyyy hh:mm | test_admin | Modify configuration for external module "monitoring_qr_v1.1.0" for project |
      | mm/dd/yyyy hh:mm | test_admin | Enable external module "monitoring_qr_v1.1.0" for project                   |
      | mm/dd/yyyy hh:mm | test_admin | Enable external module "monitoring_qr_v1.1.0" for system                    |
